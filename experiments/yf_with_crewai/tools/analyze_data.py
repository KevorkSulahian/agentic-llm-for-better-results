...
def yf_fundamental_analysis(ticker: str):
    ...
    try:
        ...
        free_cash_flow = (
            cash_flow.loc["Free Cash Flow"].iloc[0] if "Free Cash Flow" in cash_flow.index else None
        )
        ...
        def simple_dcf(fcf, growth_rate, wacc, years=5):
            ...
            dcf_value = sum(
                [fcf * (1 + growth_rate) ** i / (1 + wacc) ** i for i in range(1, years + 1)]
            )
        ...
        analysis = {
            ...
            "Last Updated": datetime.fromtimestamp(info.get("lastFiscalYearEnd", 0)).strftime(
                "%Y-%m-%d"
            ),
